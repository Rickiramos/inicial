/*function adicionar(produto){
    const qtd = document.getElementById('qtd_' + produto);
    const valor = document.getElementById('valor_' + produto);
    const total = document.getElementById('total_' + produto);

    qtd.innerHTML++
    total.innerHTML = qtd.innerHTML * valor.innerHTML
 
}

function retirar(produto){
    const qtd = document.getElementById('qtd_' + produto);
    const valor = document.getElementById('valor_' + produto);
    const total = document.getElementById('total_' + produto);

    qtd.innerHTML--
    total.innerHTML = qtd.innerHTML * valor.innerHTML

}
campactacao das duas funcoes em uma unica usando dois parametros para identificar
o produto e a ação(adicionar ou retirar). Se a acaao for igua a mais vc faz o mais
se não o menos
*/

function alterarQtd(produto, acao){
    const qtd = document.getElementById('qtd_' + produto);
    const valor = document.getElementById('valor_' + produto);
    const total = document.getElementById('total_' + produto);
   
if(acao == '-' && qtd.innerHTML == 0){
    alert("Atenção: adicione produtos")
}else{
    acao == '+' ? qtd.innerHTML++ : qtd.innerHTML --
    total.innerHTML = qtd.innerHTML * valor.innerHTML
    soma()
    
}
console.log(valor.innerHTML);

function soma(){
    let soma = 0

    for(let i = 1; i <4; i++){
        soma += Number(document.getElementById('total_' + i).innerHTML)
    }
      
    document.getElementById('subtotal').innerHTML = soma
} 

}
   